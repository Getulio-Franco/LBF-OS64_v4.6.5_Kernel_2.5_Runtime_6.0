 ...
 * ============================================================================ */
#define SFX_SHORT_SAMPLES 6000     // Aumentado (era 5000)
#define SFX_MEDIUM_SAMPLES 12000   
#define SFX_LONG_SAMPLES  20000    // Aumentado (era 18000)
...
void Init_Audio_SFX(void) {
    ...
    // ============================================================
    // 1. Som de Movimento: Tique curto e seco (MELHORADO)
    // ============================================================
    for (int i = 0; i < SFX_SHORT_SAMPLES; i++) {
        float freq = 700.0f - (300.0f * ((float)i / SFX_SHORT_SAMPLES));
        ...
        if (i < 80) {
            ...
            envelope = (1.0f - (float)(i - 80) / (SFX_SHORT_SAMPLES - 80));
        }
        if (envelope < 0) envelope = 0;
        float harmonic = 0.15f * custom_sinf(i * rad_step * 2.0f);
        int16_t val = (int16_t)((custom_sinf(i * rad_step) + harmonic) * 8000.0f * envelope * envelope);
        sfx_move[i*2] = val; sfx_move[i*2+1] = val;
    }
    
    // ============================================================
    // 2. Som do Tiro (Jogador): Impacto agudo (MELHORADO)
    // ============================================================
    for (int i = 0; i < SFX_SHORT_SAMPLES; i++) {
        ...
        float envelope;
        if (i < 60) {
            envelope = (float)i / 60.0f;
        } else {
            envelope = (1.0f - (float)(i - 60) / (SFX_SHORT_SAMPLES - 60));
        }
        if (envelope < 0) envelope = 0;
    ...
    // ============================================================
    // 3. Som do Tiro (Alien): Mais grave e com mais presença
    // ============================================================
    for (int i = 0; i < SFX_MEDIUM_SAMPLES; i++) {
        ...
        float envelope;
        if (i < 120) {
            envelope = (float)i / 120.0f;
        } else {
            envelope = (1.0f - (float)(i - 120) / (SFX_MEDIUM_SAMPLES - 120));
        }
        if (envelope < 0) envelope = 0;
    ...
    // ============================================================
    // 4. Hit Alien: Explosão com harmônicos (MELHORADO)
    // ============================================================
    for (int i = 0; i < SFX_MEDIUM_SAMPLES; i++) {
        ...
        if (i < 200) {
            ...
            envelope = (1.0f - (float)(i - 200) / (SFX_MEDIUM_SAMPLES - 200));
        }
        if (envelope < 0) envelope = 0;
        float harmonic1 = 0.3f * custom_sinf(i * rad_step * 1.5f);
        float harmonic2 = 0.15f * custom_sinf(i * rad_step * 2.5f);
        int16_t val = (int16_t)((custom_sinf(i * rad_step) + harmonic1 + harmonic2) 
                              * 16000.0f * envelope * envelope);
        sfx_alien_hit[i*2] = val; sfx_alien_hit[i*2+1] = val;
    }

    // ============================================================
    // 5. Hit Barreira: Som grave e abafado (MELHORADO)
    // ============================================================
    for (int i = 0; i < SFX_SHORT_SAMPLES; i++) {
    ...
    // ============================================================
    // 6. Hit Jogador: Som grave com modulação (MELHORADO)
    // ============================================================
    for (int i = 0; i < SFX_LONG_SAMPLES; i++) {
        ...
        if (i < 300) {
            ...
            envelope = (1.0f - (float)(i - 300) / (SFX_LONG_SAMPLES - 300));
        }
        if (envelope < 0) envelope = 0;
...
// Toca um som com controle de cooldown e busy
...
// Toca som de movimento (com cooldown)
...
// Toca som de tiro (com cooldown reduzido)
...
void Update_Game(void) {
    ...
    // 2. Movimentação e Tiro dos Aliens
    alien_move_timer++;
