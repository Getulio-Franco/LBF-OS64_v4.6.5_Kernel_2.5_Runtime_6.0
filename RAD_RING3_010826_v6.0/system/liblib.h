...
static inline __attribute__((always_inline)) int sys_audio_play(void* buffer, uint32_t size, bool loop) {
    return (int)_syscall(SYS_AUDIO_PLAY, (uint64_t)buffer, (uint64_t)size, (uint64_t)loop);
} 

// Interrompe qualquer reprodução em andamento.
static inline __attribute__((always_inline)) int sys_audio_stop(void) {
    return (int)_syscall(SYS_AUDIO_STOP, 0, 0, 0);
} 
